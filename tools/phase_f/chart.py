"""Phase F per-event chart builder -- asymmetric LULD (upper band only, N=1).

Derived from tools/phase_e/chart.py. Changes from Phase E:
  - Lower-band trigger line removed (lower band disabled in Phase F).
  - Lower-band raw band line removed.
  - luld_lower exit markers removed (never fire in Phase F).
  - Title updated to show Phase F.

Panel layout (shared x-axis):
    1. 10s OHLCV candlesticks + entry/exit markers
       + upper LULD trigger (upper_band - N*spread, orange dashed)
       + upper_band itself (orange dotted)
...
def _compute_luld_upper(
    replay: EventReplay,
    qd,
    n_spread_multiple: float,
) -> tuple[np.ndarray, np.ndarray]:
    """Replay LULD proximity -- upper band and trigger only (Phase F: lower disabled).
    ...
    if qd is None:
        return upper_band_arr, upper_trigger_arr
...
def build_chart(
    ...
    n_spread_multiple: float = 1.0,
    qd=None,
) -> go.Figure:
    """Build a 4-panel Phase F per-event chart (upper-band LULD only)."""
    ...
    upper_band, upper_trig = _compute_luld_upper(replay, qd, n_spread_multiple)
    ...
    title = (
        f"{ticker} {date} -- Phase F upper-LULD N={n_spread_multiple:.0f} | "
        f"n={len(trades_df)} ({n_first}f+{n_reentry}re) | "
        f"{n_blocks_total} blocked | "
        f"luld_hi={n_luld_upper} | "
    ...
    fig = make_subplots(
        ...
        subplot_titles=[
            "Price + LULD Upper Band + Window High",
            "I(t) sell",
    ...
    # ── Panel 1: Price + LULD upper band + cwh step line ─────────────────────

    # Upper band raw (orange dotted, lighter)
    ub_valid = ~np.isnan(upper_band)
    if ub_valid.any():
        fig.add_trace(
            go.Scatter(
                x=ts_dt[ub_valid], y=upper_band[ub_valid],
                mode="lines",
    ...
    # Upper trigger (orange dashed)
    ut_valid = ~np.isnan(upper_trig)
    if ut_valid.any():
        fig.add_trace(
            go.Scatter(
                x=ts_dt[ut_valid], y=upper_trig[ut_valid],
                mode="lines",
    ...
    if len(trades_df) > 0:
        ...
        # Non-luld_upper exits (triangles)
        non_luld_upper = exit_reasons != "luld_upper"
        if non_luld_upper.any():
            colors = ["#00CC44" if p > 0 else ("#CC2200" if p < 0 else "#888888")
                      for p in pnl[non_luld_upper]]
            custom = np.column_stack([pnl[non_luld_upper], exit_reasons[non_luld_upper]])
            fig.add_trace(
                go.Scatter(
                    x=ex_dt[non_luld_upper], y=exit_prices[non_luld_upper],
                    mode="markers",

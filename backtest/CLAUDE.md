...
**What's next (EPG-Rapid):** C0 scanner floor fix complete. R0 and R1 prior results **invalidated** (see INVALIDATED.md markers). Prior PF=2.077 was inflated by pre-scanner entries. Post-fix A4 run: PF=0.8277 — escalation triggered. **Next (after Cooper approves Part A docs):** Part B — build MDR≥150 diagnostic sample (100 events, `mom_pct ≥ 150`, confirmed scanner hit). Then R0 restart on MDR≥150. C2 reinstated (LULD independent thresholds), R4 reinstated (LULD tuning). Phase sequence: C2 → C3 ✅ → C4 ✅ → R0 (MDR≥150) → R1 → R3 → R4 → R5. Cooper sets `max_entry_lag_sec` before R1 (based on R0 T7 entry lag distribution).

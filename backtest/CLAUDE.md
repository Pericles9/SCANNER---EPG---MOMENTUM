...
| Phase EPG-Rapid C2 | **DROPPED** | LULD exit not in EPG-Rapid exit stack. `LuldProximityExit` retained unchanged for classic EPG runner only. |
...
| Phase EPG-Rapid R0 | **INVALIDATED — restart pending on MDR≥150** | Prior results (n=83, PF=2.077) generated before C0 scanner floor fix — pre-scanner entries in 65.4% of events. Post-fix PF=0.8277 — escalation. **R0 restart on MDR≥150 diagnostic sample** (100 events, `mom_pct ≥ 150`, confirmed scanner hit) pending Cooper approval of Part A docs and Part B sample build. See `results/phase_r0/INVALIDATED.md`. |
| Phase EPG-Rapid R1 | **INVALIDATED — blocked on R0 restart** | All prior R1 results (symmetric sweep, asymmetric sweep, T3 charts) generated before C0 fix. Invalid. Do not re-run until R0 restart on MDR≥150 is approved. Cooper must set `max_entry_lag_sec` (from R0 T7 distribution) before R1 begins. See `results/phase_r1/INVALIDATED.md`. |

**What's next (EPG-Rapid):** MDR≥150 diagnostic sample built (100 events, `data/val_mdr150_diagnostic.json`). LULD dropped from EPG-Rapid exit stack (C2 DROPPED, R4 DROPPED). Exit stack: EPG window close only (EXIT_D off, LULD off). Phase sequence: C3 ✅ → C4 ✅ → **R0** → R1 → R3 → R5. R0 is next. Cooper sets `max_entry_lag_sec` after R0 T7 entry lag distribution before R1 begins.

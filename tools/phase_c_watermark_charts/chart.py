"""Phase C per-event chart builder — watermark filter variant (4-panel layout).

Panel layout (shared x-axis):
    1. 10s OHLCV candlesticks + entry/exit markers
    2. Sell intensity I(t) + EXIT_D fire markers
    3. Drawdown from high watermark since T_event + threshold line + blocked entry markers
    4. EPG state coloured band strip

Public API:
    build_chart(ticker, date, replay, trades_df, event_meta, watermark_threshold) -> go.Figure
"""
...
def _compute_watermark_drawdown(replay: EventReplay) -> np.ndarray:
    """Running drawdown from the post-T_event price high watermark.

    Returns array of same length as replay timestamps, NaN before T_event fires.
    drawdown[i] = (running_high - price[i]) / running_high
    """
    ...
    t_event_idx = int(np.searchsorted(replay.timestamps_ns, replay.t_event_ns, side="left"))
    if t_event_idx >= N:
        return drawdown

    running_high = float(replay.prices[t_event_idx])
    for i in range(t_event_idx, N):
        p = float(replay.prices[i])
        if p > running_high:
            running_high = p
        drawdown[i] = (running_high - p) / running_high if running_high > 0 else 0.0
    return drawdown


def _find_blocked_entry_times(replay: EventReplay, trades_df: pd.DataFrame) -> np.ndarray:
    """Return timestamps (ns) of rising edges that were blocked by the watermark filter.

    Blocked = pass_window_open_ts entries (rising edges) with no corresponding
    first-entry in trades_df. For the watermark-only run (no CVD, no gap gate),
    any unmatched rising edge was blocked by the watermark.
    """
    if len(replay.pass_window_open_ts) == 0:
        return np.array([], dtype=np.int64)

    rising_edges = set(int(t) for t in replay.pass_window_open_ts)
    first_entries = set(
        trades_df.loc[trades_df["entry_type"] == "first", "entry_ts"]
        .astype(np.int64).values
    )
    blocked = sorted(rising_edges - first_entries)
...
def build_chart(
    ...
    event_meta: dict,
    watermark_threshold: float = 0.05,
) -> go.Figure:
    """Build a 4-panel Phase C watermark per-event chart."""
    ts_dt = pd.to_datetime(replay.timestamps_ns, unit="ns", utc=True)
    ohlcv = _build_ohlcv_10s(replay.timestamps_ns, replay.prices)
    wm_drawdown = _compute_watermark_drawdown(replay)
    blocked_ts = _find_blocked_entry_times(replay, trades_df)
    ...
    n_wm_blocks = event_meta.get("n_watermark_blocks", len(blocked_ts))
    ...
    title = (
        f"{ticker} {date} — Phase C Watermark | "
        f"n={n_trades} ({n_first} first + {n_reentry} re-entries) | "
        f"{n_wm_blocks} WM-blocked | "
        f"PF={pf_str} | wm={watermark_threshold:.0%} | {session}"
    )
    ...
    fig = make_subplots(
        ...
        subplot_titles=["Price", "I(t) sell", "Drawdown from High Watermark", "EPG state"],
    )

    # Anchor traces so add_vrect resolves y-axis refs correctly
    ...
    # ── Panel 3: Watermark drawdown ─────────────────────────────────────────
    valid = ~np.isnan(wm_drawdown)
    if valid.any():
        wm_ts = ts_dt[valid]
        wm_vals = wm_drawdown[valid]

        fig.add_trace(
            go.Scatter(
                x=wm_ts, y=wm_vals, mode="lines",
                line=dict(color="#9933CC", width=1.5),
                name="Drawdown from high",
                hovertemplate="WM drawdown %{x|%H:%M:%S}<br>%{y:.2%}<extra></extra>",
            ),
            row=3, col=1,
        )
        ...
        fig.add_trace(
            go.Scatter(
                x=[wm_ts[0], wm_ts[-1]], y=[watermark_threshold, watermark_threshold],
                mode="lines",
                line=dict(color="#CC2200", dash="dash", width=1.5),
                name=f"WM threshold {watermark_threshold:.0%}", hoverinfo="skip",
            ),
            row=3, col=1,
        )

    # Blocked entry markers on Panel 3
    if len(blocked_ts) > 0:
        blocked_dt = pd.to_datetime(blocked_ts, unit="ns", utc=True)
        block_idx = np.clip(
            np.searchsorted(replay.timestamps_ns, blocked_ts, side="left"),
            0, len(wm_drawdown) - 1,
        )
        block_dv = np.where(np.isnan(wm_drawdown[block_idx]),
                            watermark_threshold, wm_drawdown[block_idx])
        fig.add_trace(
            go.Scatter(
                x=blocked_dt, y=block_dv,
                mode="markers",
                marker=dict(symbol="x", size=12, color="#CC2200",
                            line=dict(color="#660000", width=2)),
                name="Blocked by WM",
                hovertemplate=(
                    "WM blocked<br>%{x|%H:%M:%S}<br>drawdown=%{y:.2%}<extra></extra>"
                ),
            ),
            row=3, col=1,
        )

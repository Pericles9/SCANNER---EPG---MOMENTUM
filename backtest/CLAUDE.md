...
| Phase EPG-Rapid R0 | **Complete (2026-06-22)** | MDR≥150 sample (n=100, 96 processed). T4 baseline (rising-edge): PF=3.20, n=93. T5 rapid (first-pass): PF=2.97, n=96, win%=55.2%, median_lag=306s. T6: 0 pre-scanner entries. See `results/phase_r0_mdr150/summary.md`. Old invalidated results: `results/phase_r0/`. |
| Phase EPG-Rapid R1 | **In progress** | Gate threshold sweep (p_open × p_close). `max_entry_lag_sec=300` (Cooper decision 2026-06-22 — 46/96 trades kept, PF=2.811). Expected prior: low p_open / high p_close. Results: `results/phase_r1_mdr150/`. |

**What's next (EPG-Rapid):** R0 complete. R1 gate sweep in progress. `max_entry_lag_sec=300` set by Cooper. Phase sequence: C3 ✅ → C4 ✅ → R0 ✅ → **R1** → R3 → R5.

"""Phase D per-event chart runner — intra-window watermark variant.

For each event with trades in results/phase_d/sweep_0_02/:
  1. Load EventReplay from replay cache
  2. Build 4-panel chart (price + window high, I(t), intra-window drawdown, EPG state)
  3. Write standalone HTML to results/phase_d/event_charts/

Writes sortable index.html at the end.

Usage:
    python -m tools.phase_d.run_charts
    python -m tools.phase_d.run_charts --tickers SVRE XBP --dry-run
"""
...
from tools.phase_d.chart import build_chart

_ROOT = Path(__file__).resolve().parents[2]
_RESULTS_DIR = _ROOT / "results" / "phase_d" / "sweep_0_02"
...
_OUT_DIR = _ROOT / "results" / "phase_d" / "event_charts"
...
<head>
<meta charset="utf-8">
<title>Phase D Intra-Window Watermark 2% -- Per-event charts</title>
<style>
...
<body>
<h1>Phase D Intra-Window Watermark 2% -- Per-event charts</h1>
<div class="meta">{n_events} events with trades &middot; intra_window_wm=2% &middot; click column header to sort</div>
<table id="t">
  <thead>
    <tr>
...
def _row_html(ticker: str, date: str, trades_df: pd.DataFrame,
              n_first_blocked: int, n_re_blocked: int,
              chart_filename: str) -> str:
    n_t = len(trades_df)
...
def _write_index(rows_html: list[str], n_events: int, out_path: Path) -> None:
    out_path.write_text(
        _INDEX_TEMPLATE.format(n_events=n_events, rows="\n".join(rows_html)),
        encoding="utf-8",
    )
...
def main(tickers: list[str] | None = None, dry_run: bool = False) -> None:
    ...
    for idx, (ticker, date) in enumerate(events, 1):
        ...
        try:
            ...
            fig = build_chart(ticker, date, replay, trades_slice, blocked_slice,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Phase D per-event chart runner")

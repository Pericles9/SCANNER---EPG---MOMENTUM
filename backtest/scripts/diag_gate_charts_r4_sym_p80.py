#!/usr/bin/env python3
"""
Phase R1-FINAL — Gate intensity charts for val_r4_stratified, sym_p80 config.

Copy of diag_gate_charts.py adapted for:
  - val_r4_stratified.json (100 events, stratum replaces slot)
  - p_open = p_close = 0.80 (sym_p80 best config)
  - Entry/exit from phase_r1_final/sym_p80/per_trade.json
  - Failure reason for non-traded from phase_diag_entry_r4/entry_audit.json
  - Output: phase_r1_final/event_charts_sym_p80/

Usage:
    python backtest/scripts/diag_gate_charts_r4_sym_p80.py
    python backtest/scripts/diag_gate_charts_r4_sym_p80.py --overwrite
"""
...
OUT_ROOT   = RESULTS / "phase_r1_final" / "event_charts_sym_p80"
...
def _load_trade_index() -> dict[tuple[str, str], dict]:
    """Return {(ticker, date): per_trade row} from sym_p80/per_trade.json."""
    idx = {}
    if PER_TRADE.exists():
...
def _make_index(events_meta: list[dict]) -> str:
    ...
    return textwrap.dedent(f"""\
        ...
        <head>
          <meta charset="utf-8">
          <title>R1-Final sym_p80 charts</title>
          <style>
        ...
        <body>
          <h2>Phase R1-Final — sym_p80 Gate Intensity Charts</h2>
...
def main():
    ...
    print(f"Phase R1-Final sym_p80 — gate charts for {total} events ({n_traded} traded)")

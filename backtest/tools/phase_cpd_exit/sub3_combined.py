"""
Phase CPD-EXIT Sub-Phase 3 — Combined WJI Trailing Stop + Momentum Drop
========================================================================
Both exit mechanisms active simultaneously, on top of BOCPD winner entry gate.

Entry gate: BOCPD winner config (lambda_h=0.01, p_enter=0.60).
Sweep: wji_p_exit ∈ {0.1, 0.2, 0.3, 0.5} × mom_threshold ∈ {0.04, 0.06, 0.08, 0.10, 0.12}
       = 20 configs (raw WJI variant only — log underperformed in Sub-2a).
Exit priority (first fires wins):
  1. WJI trailing stop (raw): WJI(t) < p_exit × WJI_peak_since_entry
  2. Momentum drop:           momentum_120s(t) < -mom_threshold
  3. LULD upper:              RTH upper-band proximity (N=1, Phase F config)
  4. EPG window close:        PASS → not-PASS gate transition

WJI_peak_since_entry initialised at WJI(entry tick), updated each tick.
momentum_120s(t) uses active-seconds axis (halt time excluded).
EXIT_D remains permanently disabled.

Primary benchmark to beat: Sub-Phase 1 (tp5_sl5): PF=1.399, CVaR5=-9.14%, EV=0.505

Outputs:
  results/phase_cpd_exit/sub3_combined/sweep_results.json
  results/phase_cpd_exit/sub3_combined/sweep_summary.html
  results/phase_cpd_exit/sub3_combined/event_charts/{TICKER}_{DATE}.html
  results/phase_cpd_exit/sub3_combined/event_charts/index.html

Run:
  "D:/Trading Research/.venv/Scripts/python.exe" -m tools.phase_cpd_exit.sub3_combined
"""
...
# ── Combined sweep grid ───────────────────────────────────────────────────────
WJI_PE_GRID      = [0.1, 0.2, 0.3, 0.5]     # raw WJI trailing stop p_exit values
MOM_TH_GRID      = [0.04, 0.06, 0.08, 0.10, 0.12]  # momentum drop thresholds
MOM_LOOKBACK_SEC = 120.0                      # fixed per spec

# ── Paths ─────────────────────────────────────────────────────────────────────
OUT_DIR     = REPO_ROOT / "results" / "phase_cpd_exit" / "sub3_combined"
...
SUB2B_BASELINE = {"config": "mom6",          "pf": 1.1417, "n_trades": 687,
                  "cvar5_pct": -12.60,       "ev": 0.220}
...
def build_grid() -> list[dict]:
    return [
        {"config_id":    f"pe{int(pe*100)}_mo{int(mo*100)}",
         "wji_p_exit":   float(pe),
         "mom_threshold": float(mo)}
        for pe in WJI_PE_GRID for mo in MOM_TH_GRID
    ]


# ══════════════════════════════════════════════════════════════════════════════
#  Per-event worker  (all 20 configs, 4-pass + precomputed momentum)
# ══════════════════════════════════════════════════════════════════════════════

def combined_sweep_worker(args: dict) -> dict:
    ...
    try:
        ...
        # ── Pass 2b: precompute 120s momentum (vectorised) ───────────────────
        ...
        # ── Pass 4: replay combined exit stack per config ─────────────────────
        config_results: dict[str, list[dict]] = {}
        for cfg in configs:
            p_exit    = cfg["wji_p_exit"]
            mom_th    = cfg["mom_threshold"]
            trades: list[dict] = []
            ...
            for sig_i, entry_i in entries:
                ...
                for j in range(entry_i, N):
                    ...
                    # Update running WJI peak
                    if wji_j > wji_peak:
                        wji_peak = wji_j

                    # Priority 1: WJI trailing stop (raw)
                    if wji_j < p_exit * wji_peak:
                        exit_type  = "wji_trailing_stop"
                        exit_price = float(prices[j]); exit_i = j; break

                    # Priority 2: Momentum drop
                    mom_j = mom_120s[j]
                    if not math.isnan(mom_j) and mom_j < -mom_th:
...
def _write_summary_html(rows: list[dict], path: Path, best_id: str) -> None:
    cols = [
        ("config_id", "config"), ("wji_p_exit", "wji_pe"), ("mom_threshold_pct", "mom_th%"),
        ("pf", "PF"), ("n_trades", "n"), ("win_rate", "win%"),
        ("mean_hold_sec", "hold_s"), ("cvar5_pct", "CVaR5"),
        ("ev", "EV/trade"), ("wji_stop_pct", "wji%"),
        ("mom_drop_pct", "mom%"), ("luld_pct", "luld%"), ("epg_pct", "epg%"),
    ]
...
<title>Phase CPD-EXIT Sub-Phase 3 — Combined Sweep</title>
<style>
...
<body>
<h2>Phase CPD-EXIT Sub-Phase 3 — Combined WJI Trailing Stop + Momentum Drop (20 configs)</h2>
<p>Primary target: Sub-1 (tp5_sl5) PF={SUB1_BASELINE['pf']:.4f} CVaR5={SUB1_BASELINE['cvar5_pct']:.2f}% EV={SUB1_BASELINE['ev']:.4f} &middot;
Sub-2a (raw_pe20) PF={SUB2A_BASELINE['pf']:.4f} CVaR5={SUB2A_BASELINE['cvar5_pct']:.2f}% &middot;
Sub-2b (mom6) PF={SUB2B_BASELINE['pf']:.4f} CVaR5={SUB2B_BASELINE['cvar5_pct']:.2f}% &middot;
click header to sort</p>
...
def _build_combined_chart(
    ...
    # OHLCV candles + approximate 120s momentum from candle midprices
    ...
    fig = make_subplots(
        ...
        subplot_titles=(
            "Price (10s candles)",
            f"WJI trailing floor  [p_exit={wji_p_exit:.1f}, raw]",
            f"120s price momentum  [exit threshold = −{mom_threshold*100:.0f}%]",
    ...
    # Panel 2: WJI raw + per-trade trailing floor lines
    dt, dw = _downsample(tv, wraw)
    fig.add_trace(go.Scatter(x=dt, y=dw, mode="lines",
        line=dict(color="#64b5f6", width=0.8, dash="dot"), showlegend=False), row=2, col=1)
    ...
    if t_event_ns is not None and trades:
        for tr in trades:
            ...
            xi = min(xi, len(all_wraw) - 1); ei = max(ei, 0)
            if ei >= xi:
                continue
            wji_slice   = all_wraw[ei : xi + 1]
            ta_slice    = all_ta[ei : xi + 1]
            peak_so_far = np.maximum.accumulate(wji_slice)
            floor_line  = wji_p_exit * peak_so_far
            tse_slice   = ta_slice - t_event_active
            post_m      = tse_slice >= 0.0
            if post_m.sum() == 0:
                continue
            win = tr["pnl_pct"] > 0
            fc  = "#00e676" if win else "#ff5252"
            fig.add_trace(go.Scatter(x=tse_slice[post_m], y=floor_line[post_m], mode="lines",
                line=dict(color=fc, width=1.5, dash="dash"), showlegend=False), row=2, col=1)

    # Panel 3: 120s momentum
    if mom_candle_tse:
        fig.add_trace(go.Scatter(
            x=mom_candle_tse, y=mom_candle_vals, mode="lines",
            line=dict(color="#ffa726", width=1.2), showlegend=False,
        ), row=3, col=1)
    fig.add_hline(y=-mom_threshold,
        line=dict(color="#ef5350", width=1.5, dash="dash"),
        annotation_text=f"−{mom_threshold*100:.0f}%",
    ...
    fig.update_layout(
        ...
        title=dict(
            text=(f"{ticker} {date}  |  wji_pe={wji_p_exit:.1f}  mom=−{mom_threshold*100:.0f}%"
...
def _write_chart_index(rows: list[dict], path: Path, best_cfg: dict) -> None:
    ...
    cid = best_cfg['config_id']
    html = f"""<!doctype html><html><head><meta charset='utf-8'>
<title>CPD-EXIT Sub3 combined event charts</title>
...
<body>
<h2>CPD-EXIT Sub-Phase 3 — Best combined config ({cid}) event charts</h2>
...
def main() -> None:
    ...
    print(f"Sub3 combined: {len(work)} events × {len(configs)} configs", flush=True)
    ...
    # Print sweep table
    print(f"\n{'config':<16}{'wji_pe':>7}{'mom%':>6}{'PF':>8}{'n':>7}{'win%':>7}"
          f"{'CVaR5':>9}{'EV':>8}{'hold_s':>8}{'wji%':>6}{'mom%':>6}{'luld%':>6}{'epg%':>6}")
    for r in rows:
        ...
        print(f"{r['config_id']:<16}{r['wji_p_exit']:>7.1f}{r['mom_threshold']*100:>6.0f}"
              f"{pf:>8}{r['n_trades']:>7}{wr:>7}{cv:>9}{ev:>8}{hs:>8}"
              f"{bd.get('wji_trailing_stop',0):>6.1f}{bd.get('momentum_drop',0):>6.1f}"
              f"{bd.get('luld_upper',0):>6.1f}{bd.get('epg_window_close',0):>6.1f}")
    ...
    escalations = []
    if all_pf_below_1:
        escalations.append("ALL 20 PF < 1.0 — HARD STOP")
    ...
    print(f"Sub-1 target: tp5_sl5   PF={SUB1_BASELINE['pf']:.4f}"
          f"  CVaR5={SUB1_BASELINE['cvar5_pct']:.2f}%  EV={SUB1_BASELINE['ev']:.4f}")
    print(f"Sub-2a:       raw_pe20  PF={SUB2A_BASELINE['pf']:.4f}"
          f"  CVaR5={SUB2A_BASELINE['cvar5_pct']:.2f}%")
    print(f"Sub-2b:       mom6      PF={SUB2B_BASELINE['pf']:.4f}"
          f"  CVaR5={SUB2B_BASELINE['cvar5_pct']:.2f}%")

    _write_json({
        "sub1_baseline":  SUB1_BASELINE,
        "sub2a_baseline": SUB2A_BASELINE,
        "sub2b_baseline": SUB2B_BASELINE,
    ...
    for key, event_trades in sorted(trades_by_event.items()):
        ...
        try:
            _build_combined_chart(result, best_cfg["wji_p_exit"],
                                  best_cfg["mom_threshold"], out_path)
            index_rows.append({
    ...
    print("\nSub-Phase 3 complete. All three sub-phases done.")
    ...
    print("\nAwaiting Cooper final approval before full val run.")

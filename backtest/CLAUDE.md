...
| Phase EPG-Rapid R1 — D diagnostic | **Complete** | 81 Plotly HTML charts (4-panel: candlestick, E(t), Q̃, gate state) + `entry_lag_diagnosis.json`. **H1 (EventAnchor fires after scanner): 34.6% of events — minority issue; median offset −173.7s (anchor typically fires before scanner).** **H2 (Q̃ smoothing delay): NOT binding — median first_3consec_offset=299.9s (fast qualifying).** **Gate chatter = binding constraint:** every event has ≥2 PASS→FAIL transitions (range 2–35), median entry_lag=2753s, p90=14802s. Root cause: simultaneous gate PASS + Q̃ qualifying required; chatter delays that conjunction. T2 asymmetric sweep (lower p_close) directly targets chatter. Charts committed 98d6889. |

**What's next (EPG-Rapid):** R1-T2 asymmetric sweep — 8 configs with p_close < p_open to reduce gate chatter and collapse entry lag. Configs: po60_pc55, po65_pc55, po65_pc60, po70_pc55, po70_pc60, po70_pc65, po65_pc40, po65_pc50. Script: `phase_r1_t2_sweep.py`. Reports PF, n_trades, win%, CVaR5, entry_lag, chatter, pass_window. Do NOT select winners — present table to Cooper. R2 requires explicit Cooper approval.

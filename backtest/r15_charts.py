#!/usr/bin/env python3
"""Phase R1.5 T6 — per-event 4-panel charts for the T_gate=500 arm.

Panel 1: 30s OHLC + entry ▲ + reason-colored exit ▼ + EPG PASS shading
         + T_gate check vline (entry+500s) + scanner-hit vline.
Panel 2: Sell intensity I(t)=λ_sell/(λ_buy+λ_sell) + theta line.
Panel 3: λ_V/peak ratio (re-entry disabled; buy intensity omitted) + p_close=0.65 + peak=1.0.
Panel 4: EPG gate state bands.

Traces reconstructed with the runner's fixed helpers + config-exact gate (p=0.65).
Entry/exit markers from the booked t_gate_500 per_trade.json.

Run:  python -m backtest.r15_charts
"""
...
ARM_DIR = BACKTEST / "results" / "phase_r15" / "t_gate_500"
CHART_DIR = BACKTEST / "results" / "phase_r15" / "event_charts" / "t500"
...
_STATE_SOLID = {
    "INACTIVE": "rgba(190,190,190,0.85)", "WARMUP": "rgba(255,165,0,0.85)",
    "PASS": "rgba(60,180,60,0.85)", "FAIL": "rgba(210,60,60,0.85)",
}


def _bars30(ts_ns, prices, start_ns):
    BAR = 30 * NS_PER_SECOND
    if len(ts_ns) == 0:
        e = np.array([]); return np.array([], dtype=np.int64), e, e, e, e
    bi = ((ts_ns.astype(np.int64) - start_ns) // BAR).astype(np.int64)
    st, o, h, l, c = [], [], [], [], []
    for b in np.unique(bi):
        bp = prices[bi == b]
        st.append(int(start_ns + b * BAR)); o.append(float(bp[0]))
        h.append(float(np.max(bp))); l.append(float(np.min(bp))); c.append(float(bp[-1]))
...
def collect(task):
    tk = task["ticker"]; dt = task["date"]; trade = task["trade"]
    ...
    try:
        ...
        win_hi = min(float(td.t_sec[-1]),
                     max(trade["exit_t_sec"] + WIN_POST, trade["entry_t_sec"] + T_GATE + 30.0))
        t = td.t_sec
        m = (t >= win_lo) & (t <= win_hi)
        tw = t[m]; nW = len(tw); step = max(1, nW // 20000) if nW > 40000 else 1
        si = [(max(s0, win_lo), min(s1, win_hi), s) for (s0, s1, s) in _intervals(t.tolist(), states)
        ...
        bm = (bt >= win_lo) & (bt <= win_hi)
        return {
            "status": "ok", "ticker": tk, "date": dt, "trade": trade,
            "scanner_t": scanner_t, "win_lo": win_lo, "win_hi": win_hi,
        ...
        import traceback
        return {"status": "error", "ticker": tk, "date": dt, "error": str(e),
...
def render(d, out_dir):
    ...
    tr = d["trade"]; tk = d["ticker"]; dt = d["date"]
    et, xt = tr["entry_t_sec"], tr["exit_t_sec"]
    ...
    fig = make_subplots(rows=4, cols=1, shared_xaxes=True, vertical_spacing=0.03,
                        ...
                        subplot_titles=["Price (30s OHLC) + entry/exit + EPG PASS + T_gate check",
                                        ...
                                        "λ_V / peak ratio (re-entry disabled)",
                                        "EPG gate state"])
    for s0, s1, s in d["state_intervals"]:
        if s == "PASS":
            for r in (1, 2, 3):
                fig.add_shape(type="rect", x0=s0, x1=s1, y0=0, y1=1, xref="x", yref="y domain",
                              row=r, col=1, fillcolor="rgba(0,200,100,0.12)", line_width=0, layer="below")
    if d["bt"]:
    ...
    fig.add_trace(go.Scatter(x=[et], y=[ep], mode="markers+text",
                  ...
                  text=[f"Entry @ {ep:.4f}"], textposition="top center",
                  textfont=dict(size=10, color="green"), name="Entry"), row=1, col=1)
    if reason == "time_gate":
        xc, lab = "orange", f"Time Gate @ {xp:.4f}"
    elif reason == "epg_window_close":
        xc, lab = ("green" if win else "red"), f"EPG Close @ {xp:.4f}"
    else:
        xc, lab = "gray", "Session End"
    fig.add_trace(go.Scatter(x=[xt], y=[xp], mode="markers+text",
                  marker=dict(symbol="triangle-down", size=14, color=xc),
                  text=[lab], textposition="bottom center", textfont=dict(size=10, color=xc),
                  name=f"Exit ({reason})"), row=1, col=1)
    fig.add_vline(x=et + T_GATE, line=dict(color="orange", dash="dash", width=1.5),
                  annotation_text="T_gate check (+500s)", annotation_position="top", row=1, col=1)
    ...
    for s0, s1, s in d["state_intervals"]:
        fig.add_shape(type="rect", x0=s0, x1=s1, y0=0, y1=1, xref="x", yref="y domain",
                      row=4, col=1, fillcolor=_STATE_SOLID.get(s, "rgba(200,200,200,0.85)"),
                      line_width=0, layer="below")
    for s, c in _STATE_SOLID.items():
        fig.add_trace(go.Scatter(x=[None], y=[None], mode="markers",
                      marker=dict(size=10, color=c, symbol="square"), name=s), row=4, col=1)

    pnl_s = f"{pnl:+.2f}"
    fig.update_layout(
        title=dict(text=f"{tk} {dt} | T_gate=500s | {reason} | PnL: {pnl_s}% | Hold: {hold:.0f}s",
                   font=dict(size=14)),
    ...
    fig.add_annotation(text="Panel 3: λ_V/peak ratio (re-entry disabled)",
                       xref="paper", yref="paper", x=0.0, y=-0.05, showarrow=False,
...
def build_index(rows, out_dir):
    body = "\n".join(
        ...
        f'<td>{r["hold_sec"]:.0f}</td><td>{r["pnl_pct"]:+.2f}</td>'
        f'<td>{str(r["t_gate_fired"])}</td>'
        f'<td><a href="{r["ticker"]}_{r["date"]}.html" target="_blank">chart</a></td></tr>'
        for r in rows)
    html = f"""<!DOCTYPE html><html><head><meta charset="utf-8">
<title>R1.5 T_gate=500 — per-event charts</title>
<style>
...
th{{background:#37474f;color:#fff;cursor:pointer}} td:first-child,td:nth-child(2),td:nth-child(3){{text-align:left}}
...
_a=(_c===n)?!_a:true;_c=n;rs.sort(function(a,b){{var x=a.cells[n].innerText,y=b.cells[n].innerText;
var xn=parseFloat(x),yn=parseFloat(y);if(!isNaN(xn)&&!isNaN(yn))return _a?xn-yn:yn-xn;
return _a?x.localeCompare(y):y.localeCompare(x);}});rs.forEach(function(r){{t.tBodies[0].appendChild(r);}});}}
</script></head><body>
<h2>Phase R1.5 — T_gate=500s arm — per-event charts ({len(rows)} traded events)</h2>
<table id="t"><thead><tr>
<th onclick="st(0)">Ticker</th><th onclick="st(1)">Date</th><th onclick="st(2)">Exit Reason</th>
<th onclick="st(3)">Hold(s)</th><th onclick="st(4)">PnL%</th><th onclick="st(5)">T_gate fired</th><th>Chart</th>
</tr></thead><tbody>
{body}
...
def main():
    ...
    with open(EVENT_FILE) as f:
        ev = {(e["ticker"], e["date"]): e for e in json.load(f)["events"]}
    ...
    tasks = []
    for t in trades:
        meta = ev.get((t["ticker"], t["date"]))
        if meta is None:
            print(f"  WARN {t['ticker']} {t['date']} not in event file"); continue
        tasks.append({"ticker": t["ticker"], "date": t["date"], "mom_pct": meta["mom_pct"],
                      "trade": t, "fp": pep.get((t["ticker"], t["date"]), hm), "rho": rho,
                      "q_bar_cfg": qcfg, "scanner_hit_ts_ns": meta.get("scanner_hit_ts_ns")})
    print(f"T_gate=500 arm: {len(tasks)} traded events, collecting (6 workers)...")
    res = []
    ...
    for r in ok:
        ...
        tr = r["trade"]
        rows.append({"ticker": r["ticker"], "date": r["date"], "exit_reason": tr["exit_reason"],
                     "hold_sec": tr["hold_sec"], "pnl_pct": tr["pnl_pct"],
                     "t_gate_fired": tr["exit_reason"] == "time_gate"})
    rows.sort(key=lambda x: (x["exit_reason"], -x["pnl_pct"]))
